...
public class Yami : MonoBehaviour
{
    ...
    private void ApplyBuffs()
    {
        foreach (GameObject tower in towerTracker.GetTowersInRange())
        {
            ...
            if (towerInfo.MonsterInfo.type == CardDataTypes.MonsterType.Spellcaster || towerInfo.MonsterInfo.type == CardDataTypes.MonsterType.Fiend)
            {
            ...
            if(towerInfo.MonsterInfo.type == CardDataTypes.MonsterType.Fairy)
            {
                towerInfo.ReceiveBuff(new TowerBuff(gameObject, TowerBuff.BuffStat.Attack, -200));
                towerInfo.ReceiveBuff(new TowerBuff(gameObject, TowerBuff.BuffStat.Speed, -0.2f));
            }
